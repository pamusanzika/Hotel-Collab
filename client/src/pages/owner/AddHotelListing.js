...
const AddHotelListing = () => {
  ...
  const validate = () => {
    ...
    if (collabTypes.length === 0)
      errs.collabTypes = 'Select at least one collaboration type';
    if (
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // featureImage will be set server-side based on index
      ...
      setErrors({
        submit:
          err.response?.data?.error || 'Failed to create hotel. Please try again.',
      });
          ...
          {images.length > 0 && (
            <ImageGrid>
              {images.map((img, idx) => (
                ...
                >
                  <img src={img.preview} alt={`Hotel upload ${idx + 1}`} />
                  <RemoveBtn
                    onClick={(e) => {
                      e.stopPropagation();
                      removeImage(idx);
      ...
      {showPreview && (
        <PreviewOverlay onClick={() => setShowPreview(false)}>
          <PreviewBox onClick={(e) => e.stopPropagation()}>
            ...
            <PreviewTitle>Preview Your Listing</PreviewTitle>
            ...
            <PreviewField>
              <PreviewLabel>
                Images ({images.length}) — Featured image highlighted
              </PreviewLabel>
              <PreviewImageGrid>
                {images.map((img, idx) => (
