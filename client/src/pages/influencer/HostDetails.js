...
const HostDetails = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Fetch reviews for hotel owner
        if (data.hotel?.ownerId) {
          try {
            const reviewRes = await api.get(`/reviews/user/${data.hotel.ownerId}`);
  ...
  if (loading) return <LoadingState>Loading host details...</LoadingState>;
  if (error) return <ErrorState>{error}</ErrorState>;
  if (!hotel) return <ErrorState>Host not found.</ErrorState>;
  ...
  return (
    <>
      <PageHeader title="Host Details" subtitle="Full information about this host" />
      ...
      <Wrapper>
        ...
        <DetailGrid>
          ...
          {/* Star Rating */}
          <DetailCard>
            <DetailLabel>Star Rating</DetailLabel>
            <span style={{ fontSize: '0.875rem' }}>
        ...
        <ActionBar>
          <Button
            $variant="ghost"
            onClick={() => navigate('/influencer/hosts')}
          >
            Back to Hosts
          </Button>
          <Button
          ...
          <Button
            $variant="secondary"
            onClick={() =>
              navigate('/influencer/campaigns/create', {
                state: {
                  preSelectedHotel: {
                    _id: hotel._id,
                    name: hotel.name,
                    featureImage: hotel.featureImage,
                  },
                },
              })
            }
